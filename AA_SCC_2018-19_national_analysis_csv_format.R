...
# 
# removeSpaces <- function(subana.N) {
#   colnames(subana.N) <- gsub(" ", "_", colnames(subana.N))
#   rownames(subana.N) <- gsub(" ", "_", rownames(subana.N))
#   return(subana.N)
#   }
...
# asthma severity, and asthma severity vs specialist review

levlev <- cbind(levlev, FreqSum(dat, "asthmasev4.1"))
...
makeFlatNPercInf(table(dat$asthmasev4.1, dat$Specrespreview.nicename))

levlev <- cbind(levlev, makeFlatNPercInf(table(dat$asthmasev4.1, dat$Specrespreview.nicename))) 




asthmaseveritymed <- dat %>% group_by(asthmasev4.1) %>% summarise(median_spec_rev_wait_hours = niceP(summary(arrivaltospecresprevhours, na.rm = TRUE)[3]),
                                             loIQ_spec_rev_wait_hours = niceP(summary(arrivaltospecresprevhours, na.rm = TRUE)[2]),
                                             hiIQ_spec_rev_wait_hours = niceP(summary(arrivaltospecresprevhours, na.rm = TRUE)[5]))


#  mutate(comb = paste0(med, " (", loIQ, " to ", hiIQ, ")")) %>% select(Asthma.severity = asthmasev4.1, 
#                                                                       median_hours_with_IQR = comb) # %>% CP()

asthmaseveritymed_flat <- as.matrix(asthmaseveritymed[ ,2:4]) %>% matrix(nrow = 1, ncol = 9, byrow = FALSE)
colsss <- paste(rep(colnames(asthmaseveritymed)[2:4], each = 3), "time_to_spec_rev_with",
                asthmaseveritymed$asthmasev4.1, "asthma", sep = "_")

colnames(asthmaseveritymed_flat) <- colsss

levlev <- cbind(levlev, asthmaseveritymed_flat)
...
levlev <- cbind(levlev, makeFlatNPercInf(table(dat$lengthofstaydaysmediancat, dat$Peakflowonarrival)))



levlev <- cbind(levlev, makeFlatNPercInf(table(dat$Lifestatusatdischarge, dat$Peakflowonarrival)))





levlev <- cbind(levlev, makeFlatNPerc(table(dat$lengthofstaydaysmediancat, dat$steroids4hourcat)))


levlev <- cbind(levlev, makeFlatNPerc(table(dat$Lifestatusatdischarge, dat$steroids4hourcat)))



levlev <- cbind(levlev, makeFlatNPerc(table(dat$lengthofstaydaysmediancat, dat$b2agonists4hourcat)))


levlev <- cbind(levlev, makeFlatNPerc(table(dat$Lifestatusatdischarge, dat$b2agonists4hourcat)))


levlev <- cbind(levlev, makeFlatNPerc(table(dat$lengthofstaydaysmediancat, dat$Specrespreview.nicename)))                
                

levlev <- cbind(levlev, makeFlatNPerc(table(dat$Lifestatusatdischarge, dat$Specrespreview.nicename)))                

flattobacco <- makeFlatNPerc(table(dat$DCtobaconlycurrsmokers, dat$Specrespreview.nicename))
colnames(flattobacco) <- gsub("0", "no_tobacco_advice", colnames(flattobacco))
colnames(flattobacco) <- gsub("1", "tobacco_advice", colnames(flattobacco))


levlev <- cbind(levlev, flattobacco)



levlev <- cbind(levlev, makeFlatNPercnot4(table(dat$dischargebundle, dat$Specrespreview.nicename), 3))



levlev <- cbind(levlev, makeFlatNPercnot4(table(dat$dischargebundle, dat$Specrespreview.nicename), 3))

replaceFlatColname <- function(flat, zero, one) {
  
  colnames(flat) <- gsub("0", zero, colnames(flat))
  colnames(flat) <- gsub("1", one, colnames(flat))
  return(flat)
}



levlev <- cbind(levlev, replaceFlatColname(makeFlatNPerc(table(dat$Dischargeinhalercheck, dat$Specrespreview.nicename)),
                                           zero = "No_inhaler_check", one = "Inhaler_check"))


levlev <- cbind(levlev, replaceFlatColname(makeFlatNPerc(table(dat$Dischargemaintmed, dat$Specrespreview.nicename)),
                                           zero = "Maintenance_medication_not_reviewed", 
                                           one = "Maintenance_medication_reviewed"))
levlev <- cbind(levlev, replaceFlatColname(makeFlatNPerc(table(dat$Dischargeadherence, dat$Specrespreview.nicename)),
                                           zero = "No_adherence_discussed", one = "adherence_discussed"))
levlev <- cbind(levlev, replaceFlatColname(makeFlatNPerc(table(dat$DischargePAAP, dat$Specrespreview.nicename)),
                                           zero = "No_PAAP", one = "PAAP"))
levlev <- cbind(levlev, replaceFlatColname(makeFlatNPerc(table(dat$Dischargetriggers, dat$Specrespreview.nicename)),
                                           zero = "No_triggers_discussed", one = "Triggers_discussed"))
levlev <- cbind(levlev, replaceFlatColname(makeFlatNPerc(table(dat$DischargecommFU2days, dat$Specrespreview.nicename)),
                                           zero = "No_community_FU_within_2_days_request", 
                                           one = "Community_FU_within_2_days_requested"))
levlev <- cbind(levlev, replaceFlatColname(makeFlatNPerc(table(dat$Dischargespecrev4weeks, dat$Specrespreview.nicename)),
                                           zero = "No_4_week_spec_resp_rev_request",
                                           one = "4_week_spec_resp_rev_requested"))
levlev <- cbind(levlev, replaceFlatColname(makeFlatNPerc(table(dat$Dischargenone, dat$Specrespreview.nicename)),
                                           zero = "at_least_one_discharge_element", one = "No_discharge_elements"))
